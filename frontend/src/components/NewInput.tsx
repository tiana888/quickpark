...
export default function NewInput({ placeholder ,onClick }: NewDialogProps) {
  ...
  return (
    <div className="mx-auto flex justify-center items-center bg-white rounded-lg">
      <DialogContent>
        <TextField 
      ...
      <DialogActions>
        <Button onClick={()=>{if(onClick){
        onClick(textfieldRef.current?.value); // 按下 Enter 键后执行点击动作
      }}}>
          <img className="lg:h-10 lg:w-16 h-8 lg:w-14" src="./enter.png"alt="enter"/>
        </Button>
      </DialogActions>
      </div>
  );
