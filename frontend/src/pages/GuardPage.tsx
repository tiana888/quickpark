...
export default function GuardPage() {
  ...
  const Get_Space = async () => {
    ...
    const B2 = allSpaces.slice(
      allSpaces.length / 3,
      (allSpaces.length / 3) * 2,
    );
    ...
    B1.forEach((space) => {
      space.history.forEach((spaceHistory) => {
        ...
        if (
          DepDay === targetDay &&
          DepMonth === targetMonth &&
          DepYear === targetYear
        ) {
          //case1 他今天入場
          if (
            ArrDay === targetDay &&
            ArrMonth === targetMonth &&
            ArrYear === targetYear
          ) {
            //那他佔用的時間就是
  ...
  return (
    <>
      <div className="ml-6 flex items-center justify-between">
        <button onClick={() => navigate("/")} className="back-button m-0">
          {"<返回主頁面"}
        </button>
      ...
      <div className="m-4 mx-auto w-4/5 gap-4 rounded-lg border p-7">
        <>當日B1使用率</>
        <ProgressBar
          percentage={parseFloat(B1occupiedRate.toPrecision(4))}
          _text=" "
        />
        <>當日B2使用率</>
        <ProgressBar
          percentage={parseFloat(B2occupiedRate.toPrecision(4))}
          _text=" "
        />
        <>當日B3使用率</>
        <ProgressBar
          percentage={parseFloat(B3occupiedRate.toPrecision(4))}
          _text=" "
        />
      </div>
      <div className="p-4">
